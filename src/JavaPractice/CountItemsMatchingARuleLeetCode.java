...
class Solution {
    public int countMatches(List<List<String>> items, String ruleKey, String ruleValue) {
        ...
        for(int i=0;i<items.size();i++){
            List <String> newListRef = items.get(i);
            String newList[] =newListRef.toArray(new String[3]);
             if(ruleKey.equalsIgnoreCase("type")){
                     if(newList[0].equalsIgnoreCase(ruleValue)){
                         count++;
                     }
                 }
                  else if(ruleKey.equalsIgnoreCase("color")){
                if(newList[1].equalsIgnoreCase(ruleValue)){
                         count++;                         
                     }
                 }
                  else if(ruleKey.equalsIgnoreCase("name")){
             if(newList[2].equalsIgnoreCase(ruleValue)){
                         count++;
                     }
               }
        }
